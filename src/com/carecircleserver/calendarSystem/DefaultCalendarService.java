...
public final class DefaultCalendarService implements CalendarService {
    ...
    public DefaultCalendarService(CalendarDAO dao) {
        this.dao = dao;
    }

    @Override
    public boolean createAppointment(CalendarDTO dto) {
        if (dto.patientId() == null || dto.patientId().isBlank()) {
            return false;
        }
        if (dto.appointmentTime() == null || dto.appointmentTime().isBefore(LocalDateTime.now())) {
            return false;
        }
        return dao.save(dto);
    }
    ...
    @Override
    public List<CalendarDTO> listAppointmentsByPatient(String patientId) {
        return dao.findByPatientId(patientId);
    }

    @Override
    public List<CalendarDTO> listAllAppointments() {
        return dao.findAll();
    }
}
